...
for k = 1:m
    ...
    for k_episode = 1:n
        ...
        for k_state = 1:num_state-1
            if t <= no_penalty_period
                STAY_penalty = 0;
            else
                STAY_penalty = penalty_para * (t - no_penalty_period)/(deadline - no_penalty_period);
            end
            Vest_STAY = g*w_2*x(k_state) - STAY_penalty;
            ...
            v_STAY(k_episode,k_state,k) = g*state_value_current(k_state) - STAY_penalty;
            
            %codes for STAY
            while rand < Prob_STAY % choose STAY with probability of Prob_STAY
                t = t + 1;
                TDerror = - STAY_penalty + g*w_2*x(k_state) - w_2*x(k_state);
                w_2 = w_2 + a*TDerror*x(k_state);
                num_STAY(k_episode,k_state,k) = num_STAY(k_episode,k_state,k)+1;
                
                TDerror_true = - STAY_penalty + g*state_value_current(k_state) - state_value_current(k_state);
...
data.t_set = t_set;
save procra_data_staypenalty data

%
load procra_data_staypenalty
g = 0.85; % time discount factor
n = 20; % number of episodes (trials)
num_state = 5; % number of states
m = 10000;
...
% number of times of STAY across episodes (Fig 9A right)
F = figure;
A = axes;
hold on
axis([0 21 0 4]);
...
set(A,'FontSize',20);
if save_fig
    print(F,'-depsc','Fig9Aright');
end

% approximated/true value difference at the 20th episodes (Fig 9A middle)
F = figure;
A = axes;
hold on;
axis([0 5 -0.1 0.4]);
...
set(A,'FontSize',20);
if save_fig
    print(F,'-depsc','Fig9Amiddle');
end

% approximated/true value difference at the 1st episodes (Fig 9A left)
F = figure;
A = axes;
hold on;
axis([0 5 -0.1 0.4]);
