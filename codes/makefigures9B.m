...
for k = 1:m
    ...
    for k_episode = 1:n
        ...
        for k_state = 1:num_state-1
            ...
            v_STAY(k_episode,k_state,k) = g*state_value_current(k_state) - STAY_regret;
            
            %codes for STAY
            while rand < Prob_STAY % choose STAY with probability of Prob_STAY
                TDerror = - STAY_regret + g*w_2*x(k_state) - w_2*x(k_state);
                w_2 = w_2 + a*TDerror*x(k_state);
                num_STAY(k_episode,k_state,k) = num_STAY(k_episode,k_state,k)+1;
                
                TDerror_true = - STAY_regret + g*state_value_current(k_state) - state_value_current(k_state);
...
save procra_data_stayregret data

%
load procra_data_stayregret
g = 0.85; % time discount factor
n = 20; % number of episodes (trials)
num_state = 5; % number of states
m = 10000;
...
% number of times of STAY across episodes (Fig 9B right)
F = figure;
A = axes;
hold on
axis([0 21 0 5]);
...
set(A,'YTick',[0:1:5],'YTickLabel',[0:1:5]);
set(A,'FontSize',20);
if save_fig
    print(F,'-depsc','Fig9Bright');
end

% approximated/true value difference at the 20th episodes (Fig 9B middle)
F = figure;
A = axes;
hold on;
axis([0 5 -0.1 0.5]);
...
set(A,'Ytick',[-0.1:0.1:0.5], 'Yticklabel',[-0.1:0.1:0.5]);
set(A,'FontSize',20);
if save_fig
    print(F,'-depsc','Fig9Bmiddle');
end

% approximated/true value difference at the 1st episodes (Fig 9B left)
F = figure;
A = axes;
hold on;
axis([0 5 -0.1 0.4]);
